...
test('system bodies', async ({ page }) => {
  ...
  await expect(page.locator('.system__body').first()).toBeVisible()
  await page.waitForTimeout(1500)
  await page.screenshot({ path: 'screens/system-bodies.png' })
})

test('system colony', async ({ page }) => {
  test.slow()
  await useGraphics(page, 'high')
  await page.setViewportSize({ width: 1440, height: 900 })
  ...
  await expect(page.locator('.system__body').first()).toBeVisible()
  await page.waitForTimeout(1500)
  await page.screenshot({ path: 'screens/system-colony.png' })
})

test('system inherited', async ({ page }) => {
  test.slow()
  test.setTimeout(120_000)
  await useGraphics(page, 'high')
  await page.setViewportSize({ width: 1440, height: 900 })
  ...
  await expect(page.locator('.system__body').first()).toBeVisible()
  await page.waitForTimeout(1500)
  await page.screenshot({ path: 'screens/system-inherited.png' })
})

test('system mobile', async ({ page }) => {
  await useGraphics(page, 'high')
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto('/?seed=482913')
  await page.getByRole('button', { name: '×256' }).click()
  ...
  expect(overflow).toBe(0)
  await page.screenshot({ path: 'screens/system-mobile.png' })
})
